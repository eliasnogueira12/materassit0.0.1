...
function searchLocalCatalog(queryText: string, catalog: { products: any[]; problems: any[] }) {
  ...
  function expandWord(w: string): string[] {
    ...
    const last2 = w.slice(-2), last3 = w.slice(-3);
    // Plural -> singular
...
export const askAssistant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    localResults.matchedProductIds = localResults.matchedProductIds.filter(id => !skipSet.has(id));
    ...
    const ctxProds = hasMatches
      ...
      : [];
    const topProds = ctxProds.slice(0, 8).map((p, i) => {
      const price = p.promotion_active && p.promotion_price != null
...
${hasMatches ? `CATÁLOGO (produtos relacionados):
${topProds}` : "A pesquisa no catálogo não encontrou correspondência direta para o pedido."}
...
- Se não houver nada no catálogo para ajudar, sê honesto e sugere chamar um funcionário.`;
    ...
    try {
      const tools = localResults.matchedProductIds.length === 0 ? [] : [
        {
          type: "function",
          function: {
            ...
            parameters: {
              type: "object",
              properties: {
                product_ids: { type: "array", items: { type: "string" }, description: "UUIDs dos produtos." },
              },
        ...
        },
        {
          ...
          function: {
            name: "add_to_cart",
            description: "Adiciona um produto ao carrinho do cliente. Usa esta ferramenta quando o cliente disser que quer comprar um produto específico.",
            parameters: {
              type: "object",
              properties: {
                product_id: { type: "string", description: "UUID do produto. No catálogo aparece como [ID:...] — usa esse UUID exato." },
                quantity: { type: "integer", description: "Quantidade (omitir se for 1).", default: 1 },
              },
              required: ["product_id"],
              additionalProperties: false,
            },
      ...
      return { reply: localResults.reply, greeting, products: toRecommended(localResults.products, productMap), addToCart: [] };
    }
  });
