...
export const getMostFavorited = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    const latest = new Map<string, { product_id: string; product_name: string; favorited: boolean }>();
    for (const row of data ?? []) {
      const p = row.payload as { product_id?: string; product_name?: string; favorited?: boolean } | null;
