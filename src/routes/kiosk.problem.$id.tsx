  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: links } = await supabase
        ...
        .eq("problem_id", id)
        .order("display_order");
      const products = (links ?? []).map((l: any) => l.products).filter((p: any) => p && p.active);
  ...
  if (!data) {
    return (
      <div className="p-10 max-w-3xl mx-auto text-center">
        ...
        <Link
          to="/kiosk/problems"
          className="kiosk-btn mt-8 bg-primary text-primary-foreground px-6 py-3"
        >
      ...
      <section className="mt-8">
        ...
        {products.length === 0 ? (
          <p className="text-muted-foreground">
            Sem produtos associados. Peça ajuda a um funcionário.
          </p>
        ) : (
            ...
            {products.map((p: any) => (
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-2 text-sm bg-accent/20 rounded-lg px-3 py-1.5">
                  ...
                  {[p.section, p.aisle && `Corredor ${p.aisle}`, p.shelf && `Prat. ${p.shelf}`]
                    .filter(Boolean)
                    .join(" · ") || "—"}
                </div>
              </Link>
